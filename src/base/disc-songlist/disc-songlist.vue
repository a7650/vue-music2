  ...
  <div class="list">
    ...
    <ul>
      <li
        ...
        :class="{'c-song':(currentSong && song.id === currentSong.id),'rank-index':rank,'noUrl':!song.url}"
        :key="index"
        @touchstart.once="audioPlay(song)"
        @click="selectSong(song,index,filterList)"
      >
        <div class="index">
          <div class="index1">{{index+1}}</div>
          <div class="rank" v-if="rank">{{song.rank}}</div>
        </div>
        <div class="text">
          <span
            class="name"
...
ul {
  ...
  .rank-index:nth-child(1),
  .rank-index:nth-child(2),
  .rank-index:nth-child(3) {
    .index1 {
      color: rgb(255, 72, 0);
    }
  }
