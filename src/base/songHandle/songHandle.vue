<template>
  <transition name="handles">
    <div class="all" ref="all">
      <transition name="handles">
        <div class="all-handles" ref="handles" v-if="!selectOther">
          <div class="handles" v-if="!othersHander.length">
            <span
              ref="defaultHanders"
        ...
        <div v-if="selectOther" class="selectOther">
          <div class="selectAlbum" v-if="selectAlbum">
            ...
            <scroll :data="otherAlbum" class="album-content" :stopPropagation="stopPropagation">
              <div>
                <span
                  @click="addToAlbum(item.name)"
                  v-for="(item,index) in otherAlbum"
                  :key="index"
                >{{item.name}}</span>
              </div>
            </scroll>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.$refs.all.style.height = n ? `${n * 50}px` : "200px";
      this.$refs.handles.style.height = n ? `${n * 50}px` : "200px";
      if (this.playList.length) {
        this.$refs.all.style.bottom = "8%";
      }
    });
  }
};
