  ...
  <rtol>
    <div class="disc-detail">
      ...
      <header>
        ...
        <div class="title" @click="turnTop">歌单</div>
        <div class="more" @click="albumHandle">
          <i class="icon-more"></i>
        </div>
      </header>
...
export default {
  ...
  computed: {
    othersHander() {
      if (this.isCollect) {
        return [
          { type: "_deleteAlbum", mes: "取消收藏" },
          { type: "_close", mes: "取消" }
        ];
      } else {
        return [
          { type: "_collectAlbum", mes: "收藏这个歌单" },
          { type: "_close", mes: "取消" }
        ];
      }
    },
  ...
  methods: {
    ...
    _collectAlbum() {
      let m = setCollectAlbum({
        bgUrl: this.singer.imgurl || this.singer.bgUrl,
        name: this.singer.name || this.singer.creator.name,
        dissname: this.singer.dissname,
        dissid: this.singer.dissid,
        collectAlbum: true
      });
      if (m.type === 1) {
        this.isCollect = true;
