  ...
  <div class="rank" ref="rank">
    ...
    <transition name="sidebar">
      <div class="side-bar" v-if="rankBar">
        ...
        <div class="bar">
          <header>切换榜单</header>
          <scroll :data="topList" class="list-content2">
            <ul>
              <li
                v-for="(item,index) in topList"
                :key="index"
                :class="{'c-list':currentList===index}"
                @click="selectList2(item,index)"
              >{{item.topTitle | titleFilter}}</li>
            </ul>
          </scroll>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations(["SET_SINGER", "SET_RANKBAR"])
  },
  created() {
    this._getRankList();
  }
